...
class ControllerPaymentIyzicoCheckoutForm extends Controller {
        ...
        public function index() {
                ...
                $this->model_payment_iyzico_checkout_form->disableErrorSettings();
                $this->data['cart_total'] = $cart_total_amount;
                $this->data['code'] = $this->language->get('code');
                $this->data['form_class'] = $this->config->get('iyzico_checkout_form_form_class');
        ...
        public function gettoken() {
                ...
                try {
                        ...
                        $this->data['checkout_form_content'] = '';
                        $this->data['error'] = '';
                        $this->data['display_direct_confirm'] = 'no';
                        $this->data['form_class'] = $this->config->get('iyzico_checkout_form_form_class');
                        $this->data['error_page'] = $this->url->link('checkout/error');
                        ...
                        if ($cart_total_amount == 0) {
                            $this->data['display_direct_confirm'] = 'yes';
                        ...
                        $siteLang = explode('_', $order_info['language_code']);
                        if($siteLang[0] == 'tr'){
                        ...
                        if (function_exists('curl_version')) {
                                ...
                                $this->data['siteUrl'] = $this->getSiteUrl();

                                if ($response->getStatus() == "success") {
                                        $this->data['checkout_form_content'] = $response->getCheckoutFormContent();
                                } else {
                                        $this->data['error'] = !is_null($response->getErrorMessage()) ? $response->getErrorMessage() : $response->getErrorCode();
                                }
                        } else {
        ...
        public function callback() {
                ...
                try {
                        ...
                        if ($installment > 1) {
                                ...
                                $currencySymbol = $this->_getCurrencySymbol($order_info['currency_code']);
                                ...
                                `value` = '" . (float) $calculate_total . "'
                                WHERE order_id = '$order_id' AND code = 'total' ");

                                $this->db->query("UPDATE `" . DB_PREFIX . "order`
                                SET total = '" . $calculate_total . "'
                                WHERE order_id = '" . (int) $order_id . "'");

                                $this->model_checkout_order->confirm($order_id, $this->config->get('config_order_status_id'));
                                $this->model_checkout_order->update($order_id, $this->config->get('iyzico_checkout_form_order_status_id'), $message, false);
                                ...
                                $this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('iyzico_checkout_form_order_status_id'), $message, false);
                        }
        ...
        $server_conn_slug = $this->getServerConnectionSlug();    
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                ...
                if ($cart_total_amount == 0) {
                    ...
                    $this->model_checkout_order->confirm($order_id, $this->config->get('iyzico_checkout_form_order_status_id'));
                    echo true;
                } else {
                    echo false;
                }
