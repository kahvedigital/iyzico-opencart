...
class ControllerExtensionPaymentIyzicoCheckoutForm extends Controller {
        ...
        public function index() {
                ...
                if (VERSION >= '2.2.0.0'){
                    $template_url = 'extension/payment/iyzico_checkout_form';
                } else {
                    $template_url = 'default/template/extension/payment/iyzico_checkout_form';
                }
        ...
        public function gettoken() {
                ...
                try {
                        ...
                        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/iyzico_checkout_form')) {
                                $this->template = $this->config->get('config_template') . '/template/payment/iyzico_checkout_form';
                        } else {
                                $this->template = 'default/template/payment/iyzico_checkout_form';
                        }
                        $this->load->model('checkout/order');
...
$this->load->model('setting/extension');
        ...
        public function callback() {
                ...
                try {
                        ...
                        if ($installment > 1) {
                                ...
                                $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payment_iyzico_checkout_form_order_status_id'), $message, false);
                                ...
                                $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int) $order_id . "', order_status_id = '" .
                                    $this->config->get('payment_iyzico_checkout_form_order_status_id') . "', notify = '0', comment = '" .
                                    $this->db->escape($comment) . "', date_added = NOW()");
                        } else {
                                $this->model_checkout_order->addOrderHistory($response_order_id, $this->config->get('payment_iyzico_checkout_form_order_status_id'), $message, false);
                        }
        ...
        public function error() {
                ...
                if (VERSION >= '2.2.0.0'){
                    $template_url = 'error/not_found';
                } else {
                    $template_url = 'default/template/error/not_found';
                }
        ...
        $server_conn_slug = $this->getServerConnectionSlug();
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                ...
                if ($cart_total_amount == 0) {
                    ...
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_iyzico_checkout_form_order_status_id'));
                    echo true;
                } else {
                    echo false;
                }
