...
<div class="iyzico_checkout_form_confirm">
    <div class="buttons">
        <div class="right">
            <input type="button" value="<?php echo $button_confirm; ?>" id="button-confirm" class="button" data-loading-text="<?php echo $text_wait; ?>" />
        </div>
    </div>
</div>
        ...
        $.ajax({
            ...
            success: function(json) {
                ...
                if (json.display_direct_confirm == "yes") {
                    ...
                    $(".iyzico_checkout_form_payment").show();
                    $('.iyzico-payment-form-wrapper').append('<div class="warning"><button type="button" class="close" data-dismiss="alert">×</button>' + json.error + '</div>');
                    $('#iyzico-loader').css('display','none');
                }
            }
        });
